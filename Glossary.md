---
tags:
---
# Terminology

## AC (Ante Capture)
Before the capture of [[Procyon Ab|Xenos]] into the [[Procyon]] system. Used as a [[timeline]] marker, analogous to BCE. Dates are expressed as {number} AC, counting backward from the moment of capture (0 AC).

## PC (Post Capture)
After the capture of [[Procyon Ab|Xenos]] into the [[Procyon]] system. Used as a [[timeline]] marker, analogous to CE. Dates are expressed as {number} PC, counting forward from the moment of capture (0 PC).


# Eras

## Ignic Era
The first era of [[Procyon Ab|Xenos]]'s history, during which the planet formed and existed within its birth system around an unknown red dwarf star. Characterised by geological formation, cooling, and the establishment of early crustal and hydrological conditions. No life present. Ends with the ejection of [[Procyon Ab|Xenos]] from its birth system.

## Erebian Era
The second era, beginning with [[Procyon Ab|Xenos]]'s ejection from its birth system and ending at capture. Characterised by complete darkness, global ice coverage, subsurface liquid water maintained by geothermal heat, and the emergence of the first life in deep hydrothermal vent systems. Named for Erebus, the Greek primordial deity of darkness. Ends at the moment of capture.

## Photian Era
The third era, beginning at the moment of capture into the [[Procyon]] system. Light arrives on [[Procyon Ab|Xenos]] for the first time. Characterised by atmospheric transformation, surface melting, and the explosive diversification of life. Named for Phos, the Greek primordial deity of light. Currently unnamed beyond this point.


# Taxonomy

## Taxonomic System
Life on [[Procyon Ab|Xenos]] is classified using a three-tier hierarchy reflecting the fundamental levels of biological divergence observed across the Xenian biosphere. Ranks are descriptive of relationships — they do not appear in organism names directly.

| Rank | Definition | Example |
|------|------------|---------|
| Radix | The broadest division of life, defined by independent abiogenesis and fundamentally distinct biochemical architecture. Two Radices exist on [[Procyon Ab\|Xenos]], sharing no common ancestor. | [[Ferrovita]], [[Sulphovita]] |
| Domain | A major divergence within a Radix, defined by a novel biochemical mechanism or significant architectural shift. Multiple Domains may coexist within a Radix. | [[Ferrovita Firmus]], [[Sulphovita Copula]] |
| Lineage | A distinct population within a Domain, defined by specific adaptations to local chemical conditions. Multiple Lineages may coexist within a Domain. | [[Ferrovita Firmus]] Rudis, [[Sulphovita Primalis]] Purus |

Organism names are written as **Radix Domain Lineage** — for example, [[Ferrovita Firmus]] Rudis identifies the Rudis lineage within the Firmus domain of the [[Ferrovita]] radix. Rank labels are never used as prefixes in names.

## Radix
The broadest taxonomic rank on [[Procyon Ab|Xenos]]. A Radix represents an independent origin of life — a completely distinct biochemical architecture with no ancestral relationship to any other Radix. Two Radices exist on [[Procyon Ab|Xenos]], both emerging during the Erebian Era from separate hydrothermal vent cluster systems within the same hemisphere.

## Domain
The second taxonomic rank. A Domain represents a major divergence within a Radix, characterised by a novel biochemical mechanism, metabolic strategy, or architectural reorganisation sufficiently distinct to define a separate evolutionary lineage. Domains within the same Radix share a common ancestor but have diverged beyond the threshold of lineage-level variation.

## Lineage
The third and most specific taxonomic rank. A Lineage represents a distinct population within a Domain, adapted to specific local chemical conditions. Multiple Lineages within the same Domain may coexist, partitioning the chemical environment between them, or one may outcompete another to the point of extinction.
# Biology

## [[Ferrovita]]
One of the two founding domains of life on [[Procyon Ab|Xenos]]. Mineral-based, iron-sulphur lattice organisms inhabiting the deepest hydrothermal vent zones. Emerged independently during the Erebian Era. See [[Ferrovita]].

## [[Sulphovita]]
One of the two founding domains of life on [[Procyon Ab|Xenos]]. Peptide-based, sulphur-chemistry organisms inhabiting the peripheral hydrothermal vent zones. Emerged independently during the Erebian Era. See [[Sulphovita]].

## [[Ferrovita]] Firmus
The ancestral domain of [[Ferrovita]]. Occupies the deepest, hottest vent zones. Primary metabolism is iron reduction. See [[Ferrovita Firmus]].

## [[Ferrovita]] Firmus Rudis
The earliest known lineage of [[Ferrovita]] Firmus. Fully sessile, entirely dependent on rock-bound Fe³⁺ reduction, relying on chance for juvenile settlement. The first known life on [[Procyon Ab|Xenos]]. See [[Ferrovita Firmus]].

## [[Ferrovita]] Firmus Tactus
Descended from Rudis. Juveniles develop electrophoretic drift, allowing passive movement toward Fe³⁺-rich rock surfaces during the juvenile phase. See [[Ferrovita Firmus]].

## [[Ferrovita]] Firmus Mobilis
Descended from Tactus. Adults develop the ability to detach from depleted rock surfaces and relocate via electrophoresis, eliminating strict dependence on a single substrate location. See [[Ferrovita Firmus]].

## [[Ferrovita]] Novus
The derived domain of [[Ferrovita]], descended from Firmus. Occupies cooler vent margins. Primary metabolism shifts from iron reduction toward hydrogen oxidation across the Erebian Era. Represents the first major divergence event within the [[Ferrovita]] domain. See [[Ferrovita Novus]].

## [[Ferrovita]] Novus Parcus
The earliest lineage of [[Ferrovita]] Novus. Retains iron reduction as primary metabolism but develops methanogenesis as a supplementary pathway during detachment and transit. See [[Ferrovita Novus#Novus Parcus]].

## [[Ferrovita]] Novus Liber
Descended from Parcus. Methanogenesis becomes the primary metabolic pathway, with iron reduction retained opportunistically. The first fully free-floating life on [[Procyon Ab|Xenos]]. See [[Ferrovita Novus#Novus Liber]].

## [[Sulphovita Primalis]]
The ancestral domain of [[Sulphovita]]. Occupies vent-proximal zones of Pocket B. Defined by the foundational dual-pathway metabolism — H₂S oxidation as primary energy source, methane oxidation as coupled secondary pathway dependent on H₂S priming. See [[Sulphovita]].

## [[Sulphovita Primalis]] Purus
The undifferentiated ancestral lineage of [[Sulphovita Primalis]]. Chemically conservative, undiverged, and representative of the founding [[Sulphovita]] biochemical architecture. See [[Sulphovita]].

## [[Sulphovita Copula]]
The first major divergent domain of [[Sulphovita]]. Defined by the development of iron-coordinated methane activation via the Ferric coordinase peptide module, reducing or eliminating dependency on H₂S priming. Occupies peripheral, iron-rich, H₂S-poor zones of Pocket B. See [[Sulphovita Copula]].
## [[Sulphovita Copula]] Incitor
The opportunist lineage of [[Sulphovita Copula]]. Switches contextually between iron-coordinated and H₂S-primed methane activation depending on local chemical availability. See [[Sulphovita Copula]].

## [[Sulphovita Copula]] Solutus
The specialist lineage of [[Sulphovita Copula]]. H₂S oxidation modules have been completely displaced through distributional marginalisation, leaving iron-coordinated methane oxidation as the sole metabolic pathway. The first organism on [[Procyon Ab|Xenos]] to completely abandon a founding metabolic pathway. See [[Sulphovita Copula]].

## Dual Abiogenesis
The independent emergence of two genetically and biochemically unrelated domains of life — [[Ferrovita]] and [[Sulphovita]] — within the same planetary system during the Erebian Era. The two domains share no common ancestor and arose through entirely separate chemical pathways in distinct fluid environments.

## Distributional Marginalisation
The mechanism by which [[Sulphovita]] lineages lose functional metabolic pathways. Rather than gene deletion — impossible in a peptide-based architecture with no fixed genome — modules become progressively underrepresented in the peptide network through replication drift and competitive displacement until they fall below the concentration threshold required to sustain catalytic function.

## Electrophoresis
The passive movement of a charged organism through fluid along an electrochemical gradient. In [[Ferrovita]], electron accumulation from H₂ input exceeding Fe³⁺ reduction capacity generates a net negative charge across the pyrite shell, causing drift toward Fe³⁺-rich rock surfaces. First observed in [[Ferrovita Firmus]] Tactus juveniles; extended into adult life by [[Ferrovita Novus]] lineages.

## Conformational Stabilisation
The replication mechanism of [[Sulphovita]]. An existing peptide motif partially unfolds, exposing its surface topology as a scaffold for the assembly of a similar motif from free monomers in the surrounding fluid. The scaffold stabilises intermediate structures that would otherwise dissolve before achieving functional length, allowing faithful but imperfect copying of peptide motifs without a fixed genetic sequence.

## Ferric Coordinase
A peptide module unique to [[Sulphovita]] Copula lineages. Maintains a small internal Fe²⁺/Fe³⁺ redox cycle, using the transition between oxidation states to generate localised radical chemistry sufficient to destabilise the methane C-H bond. Replaces H₂S priming as the methane activation mechanism in Copula lineages.

## Methanogenesis
The metabolic pathway — H₂ + CO₂ → CH₄ + H₂O — first developed by [[Ferrovita Novus]] Parcus as a supplementary energy source during detachment and transit. Becomes the primary metabolism of [[Ferrovita Novus]] Liber. Also produced abiotically through serpentinisation across [[Procyon Ab|Xenos]]'s iron-rich crust.

## Serpentinisation
An abiotic geochemical process in which iron-rich crustal rock reacts with subsurface water to produce methane and hydrogen. Operates planet-wide on [[Procyon Ab|Xenos]] as a continuous background process independent of volcanic activity, representing a persistent chemical energy source that underpins both [[Ferrovita]] and [[Sulphovita]] metabolisms.

## Budding
The reproductive mechanism of [[Ferrovita]]. Regions of accelerated shell accrual develop outward protrusions that grow their own internal lattice structure inherited from the parent. At sufficient mass the bud generates mechanical stress exceeding the structural integrity of the connecting material and separates as an independent organism.

## Peptide Motif
The fundamental unit of [[Sulphovita]] biochemistry. Short, reusable peptide sequences that function simultaneously as structural units, catalytic agents, and carriers of hereditary information. No strict separation exists between genetic material and metabolic machinery — the same molecules perform both roles.


# [[Geology]]

## Pocket A
The fluid environment associated with [[Ferrovita]]. Strongly acidic (pH 1–2), iron-dominated, high temperature (150–250°C), and tightly coupled to mineral surfaces. See [[Hydrology]].

## Pocket B
The fluid environment associated with [[Sulphovita]]. Moderately acidic (pH 3–4), sulphur- and methane-rich, lower temperature (50–120°C), and dominated by fluid-phase chemistry. See [[Hydrology]].

## Primary Crust
The dense iron-rich basaltic crust covering the majority of [[Procyon Ab|Xenos]]'s surface. Continuously recycled by tectonic activity.

## Volcanic Highlands
Elevated platforms built from prolonged shield volcanic accumulation. The only significant landmasses on [[Procyon Ab|Xenos]].

## Shield Volcanism
The dominant volcanic style on [[Procyon Ab|Xenos]]. Low, broad structures built from highly fluid iron-rich basaltic magma, suppressed in topographic relief by [[Procyon Ab|Xenos]]'s 2.4g surface gravity. Individual shield structures may span thousands of kilometres, their flanks merging into continuous volcanic plains.

## Hydrothermal Vent System
Zones of concentrated chemical disequilibrium where crustal recycling drives superheated, mineral-rich fluid through the crust into the subsurface ocean. The sole locations on [[Procyon Ab|Xenos]] where persistent chemical gradients sufficient to sustain metabolism exist. Two vent cluster regions, both within the same hemisphere, host all known life during the Erebian Era.